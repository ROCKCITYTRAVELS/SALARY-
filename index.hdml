...
const INITIAL_EMPLOYEES = [
  ...
  { id:11, name:"Mr.Alrasik", basicSalary:12500, workingDays:29, dept:"Travels", designation:"Manager", mobile:"9003692628", empId:"RC011", joined:"2022-06-01", photo:"", role:"Manager" },
  ...
  { id:104, name:"Mr.Alrasik", basicSalary:6500, workingDays:31, dept:"Pharmaceutical", designation:"Manager", mobile:"9003692628", empId:"PH004", joined:"2022-01-01", photo:"", role:"Employee" },
...
const TR = {
  ...
  ta: {
    appName:"ராக்சிட்டி சம்பள மேலாண்மை", dashboard:"டாஷ்போர்டு", employees:"ஊழியர்கள்",
    salary:"சம்பள தாள்", attendance:"வருகை", advances:"முன்பணம்", reports:"அறிக்கைகள்",
    totalEmployees:"மொத்த ஊழியர்கள்", monthlySalary:"மாத சம்பளம்", pendingSalary:"நிலுவை",
    totalAdvance:"மொத்த முன்பணம்", addEmployee:"ஊழியர் சேர்", editEmployee:"திருத்து",
    salarySheet:"சம்பள தாள்", department:"துறை", designation:"பதவி",
    basicSalary:"அடிப்படை சம்பளம்", workingDays:"பணி நாட்கள்", presentDays:"வந்த நாட்கள்",
    collection:"கலெக்‌ஷன்", grossSalary:"மொத்த சம்பளம்", netSalary:"நிகர சம்பளம்",
    roundOff:"ரவுண்ட் ஆஃப்", totalDeduction:"மொத்த கழிவு", share:"பகிர்",
    print:"அச்சிடு", save:"சேமி", cancel:"ரத்து", delete:"நீக்கு", edit:"திருத்து",
    name:"பெயர்", mobile:"மொபைல்", empId:"ஐடி", joined:"சேர்ந்த தேதி", role:"பொறுப்பு",
    all:"அனைத்தும்", travels:"டிராவல்ஸ்", pharma:"மருந்து", darkMode:"டார்க் மோடு",
    language:"மொழி", advance:"முன்பணம்", advanceHistory:"வரலாறு",
    giveAdvance:"முன்பணம் கொடு", amount:"தொகை", reason:"காரணம்", date:"தேதி",
    markAttendance:"வருகை பதிவு", present:"வந்தார்", absent:"வரவில்லை",
    halfDay:"அரை நாள்", leave:"விடுப்பு", monthlyReport:"மாத அறிக்கை",
    salarySlip:"சம்பள சீட்டு", close:"மூடு", copied:"நகலெடுக்கப்பட்டது!", noData:"தரவு இல்லை",
    whatsapp:"வாட்ஸ்அப் அனுப்பு", sendOne:"வாட்ஸ்அப் அனுப்பு", sendAll:"அனைவருக்கும் அனுப்பு",
    bulkSend:"அனைவருக்கும் வாட்ஸ்அப்", selectAll:"அனைத்தையும் தேர்வு", clear:"அழி", preview:"முன்னோட்டம்",
    startSending:"அனுப்பு தொடங்கு", noMobile:"மொபைல் இல்லை",
  }
};
...
export default function App() {
  ...
  const delEmp = (id) => { if(!window.confirm("Delete?"))return; setEmployees(p=>p.filter(e=>e.id!==id)); showToast("🗑️ Deleted!"); };
...
Rockcity Call Taxi & Travels`;
  ...
  const sendOneWA = (emp, entry, calc, wd) => {
    if (!emp.mobile || emp.mobile.replace(/\D/g,"").length < 10) { showToast("⚠️ "+T.noMobile+": "+emp.name); return; }
  ...
  const startBulkSend = async () => {
    ...
    if (!selRows.length) { showToast("⚠️ Select employees first"); return; }
    setBulkModal(false);
    ...
    <div style={S.app}>
      ...
      <div style={S.header} className="no-print">
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button style={{ ...S.btn(dark?"#1e293b":"rgba(255,255,255,0.15)"), padding:"5px 10px", fontSize:11, border:"1px solid rgba(255,255,255,0.3)" }}
            onClick={()=>setLang(l=>l==="en"?"ta":"en")}>{lang==="en"?"தமிழ்":"EN"}</button>
        </div>
      </div>
  ...
  >
    {icon} {T[k]}
      ...
      <div style={S.body}>
              ...
              <div key={emp.id} style={{ display:"flex", gap:12, alignItems:"center", padding:"12px 14px", borderBottom:1px solid ${t.border}, background:i%2===0?t.card:t.rowAlt }}>
                ...
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  ...
                  <div style={{ display:"flex", gap:5, marginTop:6 }}>
                    <button style={S.btnSm(t.blue)} onClick={()=>openEdit(emp)}>✏️</button>
                    <button style={S.btnSm(t.red)} onClick={()=>delEmp(emp.id)}>🗑️</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {tab==="salary" && <>
          <div style={S.row} className="no-print">
            ...
            <button style={S.btn(t.navy)} onClick={()=>window.print()}>🖨️ {T.print}</button>
        ...
        {tab==="whatsapp" && <>
          <div style={{...S.card, marginBottom:14}}>
            ...
            <div style={{padding:14}}>
              ...
              <div style={S.row}>
                <button style={S.btn(WA)} onClick={selectAllBulk}>☑️ {T.selectAll}</button>
                <button style={S.btn("#94a3b8")} onClick={clearBulkSelect}>✖️ {T.clear}</button>
              ...
              return (
                <div key={emp.id} style={{display:"flex",gap:12,alignItems:"center",padding:"12px 14px",borderBottom:1px solid ${t.border},background:selected?(dark?"#0f3460":"#e6fffa"):(i%2===0?t.card:t.rowAlt),cursor:"pointer"}}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {!hasPhone && <div style={{fontSize:10,color:t.red,marginTop:2}}>⚠️ {T.noMobile}</div>}
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
        ...
        <div style={S.modal}>
          <div style={{...S.mBox,maxWidth:400}}>
                ...
                return (
                  <div key={emp.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 10px",borderBottom:1px solid ${t.border},fontSize:12}}>
                    <div>
                      ...
                      <div style={{fontSize:10,color:hasPhone?t.subtext:t.red}}>📱 {hasPhone?emp.mobile:"⚠️ "+T.noMobile}</div>
                    </div>
            ...
            <div style={{background:t.rowAlt,borderRadius:8,padding:"10px 12px",fontSize:11,color:t.subtext,marginBottom:14}}>
              ℹ️ Each WhatsApp chat opens one-by-one with a 2.5s gap. Allow pop-ups if blocked.
            </div>
      ...
      {bulkProgress && (
        ...
        <div style={S.modal}>
          <div style={S.mBox}>
            <h3 style={{margin:"0 0 14px",fontSize:15,color:t.navy}}>{editEmp?✏️ ${T.editEmployee}:➕ ${T.addEmployee}}</h3>
            {[
